...
"""
Running the script
python conservative_cropmask_remap.py --crop-mask-path Global-cropland-percentage-map.nc --target-sample-path /home/chc-shrad/DATA/SIF/fusion_SCIAMACHY_GOME-2/SIF_SCIAMACHY_GOME2_Harmonized.YYYYMM_subsetted.nc4 --output-path /home/chc-source/shrad/Scripts/Funded_projects/2026/crop_yield_forecasting/code/notebook/code/crop_mask_fraction_on_sif_grid_conservative.nc
"""
...
def open_crop_mask(crop_path: str, var_name: str = "crop_mask") -> xr.DataArray:
    ...
    da = ds[var_name]/np.round(ds[var_name].max())  # Normalize to [0,1] if not already
    rename_map = {}
...
def open_sif_coords(target_sample_path: str, lat_name: str = "lat", lon_name: str = "lon") -> tuple[xr.DataArray, xr.DataArray]:
    ds = xr.open_dataset(target_sample_path)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Glob to locate SIF files if --target-sample-path is not provided",
    )
    parser.add_argument(
        "--output-path",
    ...
    out_ds = xr.Dataset(
        ...
        attrs={
            ...
            "source_sif_grid_sample": target_sample_path,
